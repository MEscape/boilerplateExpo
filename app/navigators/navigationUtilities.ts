...
import { isAndroid, logger, StorageKeys, useIsMounted } from 'app/utils'
import * as storage from 'app/utils/storage'

import type { NavigationProps } from './AppNavigator'
import type { AppStackParamList } from './navigation.type'
...
export const navigationRef =
  createNavigationContainerRef<NavigationContainerRef<AppStackParamList>>()
 ...
 * @returns The navigation state and persistence functions.
 */
export function useNavigationPersistence() {
  ...
  const onNavigationStateChange = (state: NavigationState | undefined) => {
    ...
    if (state !== undefined) {
      ...
      storage.save(StorageKeys.NAVIGATION_PERSISTENCE, state).then()
    }
  }
  ...
  const restoreState = async () => {
    try {
      ...
      if (!initialUrl) {
        const state = (await storage.load(StorageKeys.NAVIGATION_PERSISTENCE)) as
